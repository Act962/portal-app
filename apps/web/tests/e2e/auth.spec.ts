...
test.describe.serial("autenticação", () => {
	 ...
	 */
	test("T10: admin desativa e reativa um membro", async ({ page, browser }) => {
	...
	test("T12: o painel de insights carrega, inclusive sem dado nenhum", async ({
		...
		await expect(page.getByText("Visualizações", { exact: true })).toBeVisible();
		...
		await expect(page.getByText("De onde vinha quem abriu uma matéria.")).toBeVisible();
		await expect(page.getByText("As 10 que mais prendem o leitor.")).toBeVisible();
		await expect(page.getByText("Matérias publicadas no período.").first()).toBeVisible();

		// O gráfico em si desenhou (o SVG expõe o período no rótulo acessível).
		await expect(page.getByRole("img", { name: /Visualizações por dia/ })).toBeVisible();
