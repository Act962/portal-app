	...
	return (
		<AlertDialogPortal>
			...
			<AlertDialogPrimitive.Popup
				...
				data-size={size}
				// Fica ESTREITO de propósito, ao contrário do `Dialog`: aqui não há
				// formulário, e a confirmação existe para ser lida inteira de um golpe de
				// vista. Só o teto de altura acompanha, para o texto longo de consequência
				// não empurrar os botões para fora da tela.
				className={cn(
					"group/alert-dialog-content data-open:fade-in-0 data-open:zoom-in-95 data-closed:fade-out-0 data-closed:zoom-out-95 fixed top-1/2 left-1/2 z-50 grid max-h-[calc(100dvh-2rem)] w-full -translate-x-1/2 -translate-y-1/2 gap-4 overflow-y-auto rounded-xl bg-popover p-4 text-popover-foreground outline-none ring-1 ring-foreground/10 duration-100 data-[size=default]:max-w-xs data-[size=sm]:max-w-xs data-closed:animate-out data-open:animate-in data-[size=default]:sm:max-w-md",
					className,
				)}
				{...props}
			/>
