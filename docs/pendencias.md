...
| **Periodicidade das tarefas fica em dois lugares** | O cron da Vercel lê o `vercel.json`, não o `cron` declarado na tarefa (`packages/api/src/scheduler.ts`) | Mudar a frequência num só lugar não dá erro — a tarefa simplesmente roda na frequência antiga. Some ao trocar para `node-cron` ou Inngest, que leem o registro. Ver [`deploy.md`](./deploy.md) §3 |
| **`EventBus` síncrono não tem retry** | O adapter durável (Inngest) nunca foi escrito — ver [`adr/0007`](./adr/0007-eventos-e-agendamento-atras-de-portas.md) | Um consumidor que falhe perde a rodada. O evento não some (a linha do outbox fica sem `processedAt`), mas nada tenta de novo sozinho. A porta `Scheduler` já existe, então plugar o Inngest é um arquivo na composição |
